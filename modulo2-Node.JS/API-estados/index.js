...
class Main {
  ...
  async read() {
    try {
      const readStates = JSON.parse(
        await fs.readFile(
          'D:/Breno Noccioli/Desktop/IGTI/Módulo 2/desafio_1//Estados.json'
        )
      );
      const readCities = JSON.parse(
        await fs.readFile(
          'D:/Breno Noccioli/Desktop/IGTI/Módulo 2/desafio_1/Cidades.json'
        )
      );
  ...
  async countCities(uf = 'ba') {
    ...
    const cities = JSON.parse(
      await fs.readFile('./estados/' + `${state}.json`)
    );
      ...
      this.cities.filter((city) => {
        if (city.Estado == state.ID) {
          ...
          totalCities.splice(
            index,
            1,
            `${state.Sigla}: ${stateContent.length}`
          );
        }
      });
    });

    const topFive = totalCities
      .sort((a, b) => a.replace(/\D+/g, '') - b.replace(/\D+/g, ''))
      .slice(-5)
      .reverse();
    const bottomFive = totalCities
      .sort((a, b) => a.replace(/\D+/g, '') - b.replace(/\D+/g, ''))
      .slice(0, 5)
      .reverse();
  ...
  async maxMinLetters() {
    ...
    for (let pos in citiesOfState)
      console.log(
        citiesOfState[pos].sort(
          (a, b) => a.replace(/\D+/g, '') - b.replace(/\D+/g, '')
        )[citiesOfState[pos].length - 1]
      );
  ...
  async maxMinLettersAll() {
    ...
    allCities.forEach((city) => {
      if (city.replace(/\D+/g, '') == maxLength) maxLengthCities.push(city);

      if (city.replace(/\D+/g, '') == minLength) minLengthCities.push(city);
    });
